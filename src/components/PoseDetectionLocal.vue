...
import {PoseDetector, MoveNetModelConfig} from '@tensorflow-models/pose-detection'
...
let detector: PoseDetector, requestID: any; // requestAnimationFrame
...
const startDetect = async () => {
  ...
  if (typeof webcam !== 'undefined' && webcam !== null && webcam.video!.readyState === 4) {
    ...
    // 画第一个人的姿势 poses[0]
    // 画点
    drawKeypoints(canvas!.getContext('2d'), poses[0].keypoints);
    // 画骨骼
    drawSkeleton(canvas!.getContext('2d'), poses[0].keypoints, poses.id);
